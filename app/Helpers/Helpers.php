...
class Helpers
{
    public static function processImages($imagem, $largura, $altura, $pasta)
    {
        // Recura alguns dados da imagem
        list($largura_original, $altura_original, $tipo, $atributo) = getimagesize($imagem);

        // dd(getimagesize($imagem));
        $imagem_2 = $imagem;
        // Salva a imagem e recupera o caminho completo
        $nomeImagem = Str::slug(hash::make(date('l jS \of F Y h:i:s A'))) . '.' . pathinfo($imagem_2->getClientOriginalName())['extension'];
        $arquivo = $imagem_2->move(public_path('uploads/'.$pasta), $nomeImagem);
        // $nomeImagem = str_replace('public/'.$pasta.'/', '', $imagem_2->store('public/'.$pasta));
